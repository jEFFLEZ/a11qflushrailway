...
try {
    ...
    import('./storage.cjs').then((m) => { saveEngineHistory = m && (m.saveEngineHistory || (m.default && m.default.saveEngineHistory)); }).catch(() => {
        import('./storage.js').then((m) => { saveEngineHistory = m && (m.saveEngineHistory || (m.default && m.default.saveEngineHistory)); }).catch(() => { });
    });
}
catch (e) {
...
let callReload;
try {
    import('./daemon-control.cjs').then((m) => { callReload = m && (m.callReload || (m.default && m.default.callReload)); }).catch(() => {
        import('./daemon-control.js').then((m) => { callReload = m && (m.callReload || (m.default && m.default.callReload)); }).catch(() => { });
    });
}
catch (e) {
    callReload = undefined;
    console.warn('[executor] load daemon-control failed', String(e));
}
...
function safeExecFile(cmd, cwd, timeoutMs) {
    return new Promise((resolve) => {
        ...
        if (/^[\w@.\-\/\\]+$/.test(parts[0])) {
            const child = execFile(parts[0], parts.slice(1), { cwd, env: { PATH: process.env.PATH || '' }, timeout: timeoutMs }, (err, stdout, stderr) => {
...
export async function executeAction(action, ctx = {}) {
    ...
    try {
                ...
                try {
                    if (String(cfg.webhookUrl).includes('127.0.0.1') || String(cfg.webhookUrl).includes('localhost')) {
                        ...
                        await new Promise((resolve) => {
                            ...
                            const req = httpmod.request({ hostname: u.hostname, port: u.port, path: (u.pathname || '/') + (u.search || ''), method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(body) } }, (r) => { r.on('data', () => { }); r.on('end', () => resolve(undefined)); });
        ...
        if (action.startsWith('daemon.reload')) {
            ...
            try {
                if (saveEngineHistory)
                    saveEngineHistory('reload-' + Date.now(), Date.now(), ctx.path || '', 'daemon.reload', res);
            }
            catch (e) { }
            return res;
        }
