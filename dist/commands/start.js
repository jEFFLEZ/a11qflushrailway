...
import { runCustomsCheck, hasBlockingIssues } from "../utils/npz-customs.js";
    ...
    try {
        ...
        try {
            ...
            if (fs.existsSync(spyCfgPath)) {
                ...
                if (cfg && typeof cfg.adminPort === 'number' && cfg.adminPort > 0)
        ...
        try {
            ...
            if (fs.existsSync(logicCfg)) {
                ...
                if (lc && (typeof lc.spyderAdminPort === 'number'))
                    return lc.spyderAdminPort;
...
async function isPortInUse(host, port, timeout = 400) {
        ...
        try {
            const s = net.createConnection({ host, port, timeout }, () => {
                try {
                    ...
                    console.warn('[start] isPortInUse destroy failed:', err);
                }
                resolve(true);
            });
            ...
            s.on('timeout', () => { try {
                ...
                console.warn('[start] isPortInUse destroy on timeout failed:', err);
            } ; resolve(false); });
        }
        catch (e) {
            resolve(false);
...
function persistSpyderAdminPort(spyPort) {
    try {
        ...
        try {
            if (!fs.existsSync(path.join(qflushDir, 'logs')))
                fs.mkdirSync(path.join(qflushDir, 'logs'), { recursive: true });
        }
        catch (e) { /* ignore */ }
...
export async function ensureBuiltIfNeeded(prefixPath) {
    try {
        const candidates = [
            prefixPath,
        ...
        for (const cand of candidates) {
            try {
                ...
                if (!pj || !pj.scripts || !pj.scripts.build)
                    continue;
...
export async function startWithCustoms(modName, optsParam, pathsParam) {
...
export async function resolveRunCommandForPackage(mod, pkgPath, pkgJson) {
    if (!pkgPath)
        return null;
    // 1) package bin entry
    if (pkgJson?.bin) {
...
export function spawnAndWait(mod, runCmd, waitForStart, flags) {
    return new Promise((resolve) => {
        ...
        if (waitForStart) {
            ...
            if (svcUrl) {
                waitForService(svcUrl, svcPort).then((ok) => {
                    if (ok)
                        logger.success(`${mod} passed health check`);
                    else
                        logger.warn(`${mod} failed health check`);
                    resolve();
                }).catch(() => resolve());
            }
            else {
        ...
        if (fs.existsSync(routesPath)) {
            try {
                ...
                if (Array.isArray(routesArr))
                    count = routesArr.length;
                else if (routesArr && typeof routesArr === 'object')
                    count = Object.keys(routesArr).length;
                if (count > 0) {
                    try {
                        const m = await import('../cortex/resonnance.js');
                        const startFn = (m && typeof m.resonnance === 'function') ? m.resonnance : (m && typeof m.default === 'function' ? m.default : null);
...
async function probeAndPersistSpyderPort() {
...
async function resolveOrMergedRunCmd(mod, pkg, pkgPath, pkgJson) {
    ...
    return { cmd: resolved.cmd, args: resolved.args || [], cwd: resolved.cwd };
}
...
async function resolvePackagePathForModule(mod, opts, paths) {
    ...
    if (!pkgPath && pkg) {
        const ok = ensurePackageInstalled(pkg);
        if (!ok)
            return { pkgPath: null, pkg, pkgJson: null };
        pkgPath = resolvePackagePath(pkg);
    }
    if (!pkgPath)
        return { pkgPath: null, pkg, pkgJson: null };
...
async function startRunnable(mod, runCmd, waitForStart, flags) {
    if (!runCmd)
        return;
...
export async function startModule(mod, opts, paths, flags, waitForStart) {
